...
export class WorkoutChartComponent implements OnInit, OnDestroy {
  ...
  updateCharts(users: User[]): void {
        ...
        plugins: {
          ...
          datalabels: {
            formatter: (value, context) => {
              const percentage = (value / workoutDistribution.data.reduce((a, b) => a + b, 0)) * 100;
    ...
    if (this.selectedUser) {
      const workoutTimeDistribution = this.getWorkoutTimeDistribution(this.selectedUser);
      this.doughnutChart = new Chart('doughnutChart', {
        ...
        options: {
          ...
          plugins: {
            ...
            datalabels: {
              formatter: (value, context) => {
                const percentage = (value / workoutTimeDistribution.data.reduce((a, b) => a + b, 0)) * 100;
                return percentage > 0 ? `${percentage.toFixed(1)}%` : '';
              },
              color: '#000000',
              font: {
  ...
  private prepareChartData(users: User[]): { labels: string[], datasets: any[] } {
    const workoutTypes = ['Running', 'Swimming', 'Cycling', 'Yoga', 'Weight Training'];
    ...
    const datasets = workoutTypes.map(type => ({
      ...
      backgroundColor: userData.map(user => this.getColorForWorkoutType(type)),
      borderColor: '#000000',
      borderWidth: 2
    }));
  ...
  getWorkoutDistribution(users: User[]): { labels: string[], data: number[], colors: string[] } {
    const workoutTypes = ['Running', 'Swimming', 'Cycling', 'Yoga', 'Weight Training'];
    const workoutMinutes = workoutTypes.map(type => users.reduce((sum, user) => {
      const workout = user.workouts.find(w => w.type === type);
  ...
  private getWorkoutTimeDistribution(user: User): { labels: string[], data: number[], colors: string[] } {
    const workoutTypes = ['Running', 'Swimming', 'Cycling', 'Yoga', 'Weight Training'];
  ...
  getColorForWorkoutType(type: string): string {
    switch (type) {
      case 'Running': return '#c4b484';
      case 'Swimming': return '#a3c4bc';
      case 'Cycling': return '#b0a3c4';
      case 'Yoga': return '#c4a3a3';
      case 'Weight Training': return '#a3c4a3';
